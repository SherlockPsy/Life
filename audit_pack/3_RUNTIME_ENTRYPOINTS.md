# RUNTIME ENTRYPOINTS — server.js AND CALL GRAPH

## File: server.js

**SHA-256:** `50907403d571995d581a3e142f2b3b3526cd8f3d5ac0ea5a0ca9ee136da1fd89`  
**Total Lines:** 335
        ...
        if (aiBundlePayload.wrote && aiBundlePayload.entries) {
          for (const entry of aiBundlePayload.entries) {
            ...
            entries.push({
              ...
              channel: channel,
              author: { author_id: entry.source.actor },
              text: text
            });
          }
        }
...
### Engines Imported (Lines 5-12):
| Import | Line |
|--------|------|
| Engine2 (BEAT_AND_OPPORTUNITY_COORDINATOR) | 5 |
| Engine3 (TIME_AND_CALENDAR_ENGINE) | 6 |
| Engine5 (SCENE_ANCHOR_AND_REHYDRATION_ENGINE) | 7 |
| Engine6 (CAPSULE_ENGINE) | 8 |
| Engine7 (TOOL_REQUEST_ENGINE) | 9 |
| Engine8 (RETRIEVAL_ENGINE) | 10 |
| Engine9 (LLM_WRITER_ENGINE) | 11 |
| Engine10 (WRITE_ACCEPTANCE_AND_INTEGRITY_ENGINE) | 12 |

### Engine Calls in /invocations Endpoint:
| Engine | Method | Line Range | Purpose |
|--------|--------|------------|---------|
| Engine3 | setTime | 77 | Set explicit world time override |
| Engine3 | advanceTime | 83, 88 | Advance time by delta or default +1 |
| Engine2 | handleBeat | 93 | Coordinate beat boundary |
| Engine5 | handleBeatBoundary | 99 | Check for rehydration trigger |
| Engine5 | getLatestAnchor | 103 | Get scene anchor for context |
| Engine9 | generateProposal | 173 | Generate LLM write proposal |
| Engine10 | validateWriteBundle | 179 | Validate write bundle integrity |
| Engine7 | validateToolRequest | 253 | Validate tool request |
| Engine8 | executeRetrieval | 261 | Execute ledger search/get |
| Engine6 | executeCapsule | 263 | Execute capsule retrieval |

### Endpoint Summary:
| Route | Method | Lines | Purpose |
|-------|--------|-------|---------|
| /invocations | POST | 23-287 | Main invocation endpoint (Engine 0/1 behavior) |

### Direct Database Operations (via pg Pool):
- invocations table: SELECT (L45), INSERT (L66)
- bundles table: SELECT (L48), INSERT (L136, L206)
- entries table: SELECT (L53, L112), INSERT (L160, L226)
- world_clock table: via Engine3
- beats table: via Engine2
- scene_anchors table: via Engine5
- rehydration_events table: via Engine5
